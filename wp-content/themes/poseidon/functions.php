    ...
    if (is_object($EM_Event)) {
        ...
        switch ($condition) {
            ...
            case 'has_att_website':
                if (is_array($EM_Event->event_attributes) && !empty($EM_Event->event_attributes['Event webpage']))
            ...
            // #_ATT{Ticket link}
            case 'has_att_ticket_link':
                if (is_array($EM_Event->event_attributes) && !empty($EM_Event->event_attributes['Ticket link']))
                    $replacement = preg_replace('/\{\/?has_att_ticket_link\}/', '', $match);
                else
                    $replacement = '';
                break;
            
            // #_ATT{Ticket price}
            case 'has_att_ticket_price':
                if (is_array($EM_Event->event_attributes) && !empty($EM_Event->event_attributes['Ticket price']))
                    $replacement = preg_replace('/\{\/?has_att_ticket_price\}/', '', $match);
                else
                    $replacement = '';
                break;
            ...
            // #_ATT{Event email}
            case 'has_att_email':
                if (is_array($EM_Event->event_attributes) && !empty($EM_Event->event_attributes['Event email']))
                    $replacement = preg_replace('/\{\/?has_att_email\}/', '', $match);
                else
                    $replacement = '';
                break;
            ...
            // #_ATT{Event phone}
            case 'has_att_phone':
                if (is_array($EM_Event->event_attributes) && !empty($EM_Event->event_attributes['Event phone']))
                    $replacement = preg_replace('/\{\/?has_att_phone\}/', '', $match);
                else
                    $replacement = '';
                break;
        }
